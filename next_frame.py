...
def main():
    if not os.path.exists('./nextframe_img'):
        os.mkdir('./nextframe_img')
    ...
    for epoch in range(EPOCH):
        for step, img in enumerate(data_loader):
            ...
            if step == 0:
                ...
                save_image(pic, './nextframe_img/image_{}.jpg'.format(step))
            if step > 0:
                encoded, decoded = auto_encoder(b_wait)
                loss = loss_function(decoded, b_x)
                b_wait = b_x
        ...
        torch.save(auto_encoder.state_dict(), './nextframe_autoencoder.pth')
        torch.save(auto_encoder, 'nextframe_model.pkl')
